    ...
    private companion object {
        ...
        private val STUCK = """
            ...
            AND (
                ...
                (venterPaHva in ('BEREGNING', 'UTBETALING', 'VILKÅRSPRØVING', 'INFOTRYGDHISTORIKK', 'HJELP'))
                ...
                (venterPaHvorfor IS NOT NULL AND venterPaHvorfor != 'OVERSTYRING_IGANGSATT')
                    OR
                -- Om vi venter for alltid skal alarmen gå når vi har ventet 3 måneder, så fremt det ikke venter på godkjenning fra saksbehandler, inntektsmelding eller søknad
                (venterForAlltid = true AND ventetSiden < (now() AT TIME ZONE 'Europe/Oslo') - INTERVAL '3 MONTHS' AND venterPaHva not in ('GODKJENNING', 'INNTEKTSMELDING', 'SØKNAD'))
                    OR 
