...
internal class OppsummerVedtaksperiodeVenter(
    ...
    private companion object {
        ...
        private val String.finÅrsak get() =
            replace("_", " ").lowercase().let {
                if (it.skummel) {
                    "$it $etUndrendeSmilefjes"
                } else if (it.stuck) {
                    "<https://www.nav.no/saksbehandlingstider#sykepenger|$it> :look-away:"
                } else {
                    it
                }
            }
        ...
        private fun Int.finProsentAv(total: Int) =
            ((this.toDouble() / total) * 100).let {
        ...
        private val Ventegruppe.venterPå get() =
            when {
