    ...
    override fun onPacket(packet: JsonMessage, context: MessageContext) {
        ...
        sessionOf(dataSource, returnGeneratedKey = true).use {
            it.transaction { session ->
                @Language("PostgreSQL")
                val statement = """
                     INSERT INTO revurdering_igangsatt(opprettet, fodselsnummer, aktor_id, kilde, skjaeringstidspunkt, periode_for_endring_fom, periode_for_endring_tom, aarsak)
                     VALUES (:opprettet, :fnr, :aktor_id, :kilde, :skjaeringstidspunkt, :fom, :tom, :aarsak)
                      """
                ...
                @Language("PostgreSQL")
                val statement2 = """
                    INSERT INTO revurdering_igangsatt_vedtaksperiode(revurdering_igangsatt_id, vedtaksperiode_id, periode_fom, periode_tom, skjaeringstidspunkt, orgnummer)
