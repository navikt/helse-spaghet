...
class SpaghetE2ETest {
    @Test
    fun `lagrer godkjenninger for gammel rapport`() = e2eTest {
    ...
    @Test
    fun `finner vedtaksperiode_endret for hendelse`() = e2eTest {
        val vedtaksperiodeId = UUID.randomUUID()
        val id = UUID.randomUUID()
    ...
    @Test
    fun `finner ikke vedtaksperiode_endret for hendelse som forblir i samme tilstand`() = e2eTest {
        val vedtaksperiodeId = UUID.randomUUID()
        val id = UUID.randomUUID()
    ...
    @Test
    fun `defaulter til ident om saksbehandler oid ikke eksisterer`() = e2eTest {
    ...
    @Test
    fun `defaulter til spesialist oid om det er en automatisk behandling`() = e2eTest {
        val fødselsnummer = "83497290"
        val vedtaksperiodeId = UUID.randomUUID()
        val id = UUID.randomUUID()
    ...
    @Test
    fun `godkjenningsbehov blir lest fra rapid`() = e2eTest {
    ...
    @Test
    fun `godkjenningsbehov blir lest fra rapid uten refusjonstype`() = e2eTest {
        val fødselsnummer = "1243356"
        val vedtaksperiodeId = UUID.randomUUID()
        val id = UUID.randomUUID()
        rapid.sendTestMessage(behovNyttFormat(fødselsnummer, vedtaksperiodeId, "FORLENGELSE", id))
    ...
    @Test
    fun `persisterer periodetype for nye godkjenninger`() = e2eTest {
        val id = UUID.randomUUID()
    ...
    @Test
    fun `persisterer inntektskilde`() = e2eTest {
    ...
    @Test
    fun `lagrer kun behov en gang`() = e2eTest {
    ...
    @Test
    fun `lagrer kun løsning en gang`() = e2eTest {
        val eventId = UUID.randomUUID()
        val vedtaksperiodeId = UUID.randomUUID()
    ...
    @Test
    fun `lagrer kun tilstandsendring en gang`() = e2eTest {
        val vedtaksperiodeId = UUID.randomUUID()
        val id = UUID.randomUUID()
        rapid.sendTestMessage(vedtaksperiodeEndret(id = id, vedtaksperiodeId = vedtaksperiodeId))
        rapid.sendTestMessage(vedtaksperiodeEndret(id = id, vedtaksperiodeId = vedtaksperiodeId))
    ...
    private fun E2eTestApp.finnGodkjenninger(fødselsnummer: String) = sessionOf(dataSource).use { session ->
        session.run(queryOf("SELECT * FROM godkjenning WHERE fodselsnummer=?;", fødselsnummer)
            .map { it.string("vedtaksperiode_id") }
            .asList)
    }

    private fun E2eTestApp.finnGodkjenningsbehovLøsning(id: UUID) = sessionOf(dataSource).use { session ->
        session.run(queryOf("SELECT * FROM godkjenningsbehov_losning WHERE id=?;", id)
            .map { it.string("id") }
            .asList)
    }

    private fun E2eTestApp.finnGodkjenningsbehovLøsningBegrunnelse(id: UUID) = sessionOf(dataSource).use { session ->
        session.run(queryOf("SELECT * FROM godkjenningsbehov_losning_begrunnelse WHERE id=?;", id)
            .map { it.string("id") }
            .asList)
    }

    private fun E2eTestApp.finnVedtaksperiodeTilstandsendring(id: UUID) = sessionOf(dataSource).use { session ->
        session.run(queryOf("SELECT * FROM vedtaksperiode_tilstandsendring WHERE id=?;", id)
            .map { it.string("tilstand_til") }
            .asList)
    }

    private fun E2eTestApp.finnPeriodetype(id: UUID) = sessionOf(dataSource).use { session ->
        session.run(queryOf("SELECT * FROM godkjenningsbehov WHERE id=?;", id)
            .map { it.stringOrNull("periodetype") }
            .asSingle)
    }

    private fun E2eTestApp.finnSaksbehandlerIdentitet(id: UUID) = sessionOf(dataSource).use { session ->
        session.run(queryOf("SELECT * FROM godkjenningsbehov_losning WHERE id=?;", id)
            .map { it.string("godkjent_av") }
            .asSingle)
    }

    private fun E2eTestApp.finnInntektskilde(id: UUID) = sessionOf(dataSource).use { session ->
        session.run(queryOf("SELECT * FROM godkjenningsbehov WHERE id=?;", id)
            .map { it.stringOrNull("inntektskilde") }
            .asSingle)
    }

    private fun E2eTestApp.finnGodkjenningsbehov(vedtaksperiodeId: UUID) = sessionOf(dataSource).use { session ->
